...
if (isset($_GET['id']) && filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
    $var_id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
        ...
        $sql = "SELECT * FROM composant WHERE id = ?";
...
if (isset($_POST['update'])) {
    ...
    $var_cout = clean_input($_POST['cout']);
    $var_id = clean_input($_POST['id']);

    if (empty($var_libelle) || empty($var_description) || empty($var_cout)) {
        $message = "<p style='color: red;'>Veuillez remplir tous les champs du formulaire !</p>";
    } else {
    
            $sql = "UPDATE composant SET libelle = ?, description = ? WHERE id = ?";
...
<body>
    ...
    <form action="" method="post">
        ...
        <input type="text" id="cout" name="cout" value="<?= htmlspecialchars($cout); ?>">
